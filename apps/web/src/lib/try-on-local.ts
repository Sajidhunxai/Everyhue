...
export function renderTryOnLocal(
  ...
  const scaled = downscale(Uint8Array.from(decoded.data), decoded.width, decoded.height, 512);
  const masks = buildMasks(scaled.pixels, scaled.width, scaled.height);
  const out = new Uint8Array(scaled.pixels);
  ...
  for (let i = 0, p = 0; i < w * h; i++, p += 4) {
    let r = scaled.pixels[p];
    let g = scaled.pixels[p + 1];
    let b = scaled.pixels[p + 2];
    const lum = (0.2126 * r + 0.7152 * g + 0.0722 * b) / 255;
